...
/**
 * use the Upsert policy for insert
 *
...
public class MySqlToEs_user_2 implements CloudCanalProcessorV2 {
    ...
    private final SchemaInfo factTable = new SchemaInfo(null, "middle_imall", "product_tag_mapping");
    ...
    protected void handleFactTable(List<CustomData> re, CustomData data) {
        ...
        while (it.hasNext()) {
            ...
            try (Connection conn = srcDataSource.getConnection()) {
                ...
                // Delete this data if the item is empty
                if (productName == null) {
                    it.remove();
                    continue;
                }
                ...
                if (!tags.isEmpty() && data.getEventType() == EventTypeInSdk.DELETE) {
                    // The source is deleted, but in fact the item is still associated with the tag, you must do the update
                    it.remove();
