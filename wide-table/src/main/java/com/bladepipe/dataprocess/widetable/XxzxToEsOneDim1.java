...
public class XxzxToEsOneDim1 implements CloudCanalProcessorV2 {
    ...
    private final SchemaInfo          factTable               = new SchemaInfo(null, "dingtax", "xxzx_xxzb");

    // 2nd dimension table definition

    private final SchemaInfo          dimensionTable_2        = new SchemaInfo(null, "dingtax", "bi_dm_swjg");

    // 2nd dimension table's join key definition in fact table

    private final CustomFieldV2       factTableJoinKey_2      = CustomFieldV2.buildField("xnzz_id", null, Types.BIGINT, false, false, true);

    // 2nd dimension table's join key definition in dimension table

    private final CustomFieldV2       dimensionTableJoinKey_2 = CustomFieldV2.buildField("xnzz_id", null, Types.BIGINT, false, false, true);
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        for (CustomFieldV2 addCol : addCols) {
                            // If you don't find the record change the primary key value?
    ...
    protected CustomRecordV2 genNewRecord(CustomRecordV2 origin, CustomFieldV2 joinKey, EventTypeInSdk eventType, ResultSet rs, List<CustomFieldV2> addCols,
        ...
        for (int i = 0; i < addCols.size(); i++) {
            ...
            if (val != null) {
                ...
                // Use your own primary key generation strategy
                long newPkNum = Math.abs(newPk.hashCode());

                fillColVal(newOne, "id", newPkNum);
            } else {
                // If the value of the search record is null change primary key?
                CustomFieldV2 addCol = addCols.get(i);
                CustomFieldV2 cf = CustomFieldV2.buildField(addCol.getFieldName(), null, addCol.getSqlType(), addCol.isKey(), true, true);
                newOne.addField(cf);
            }
